...
base → style → char → combo(固定 seed=42):
...
- `base`:通用插画风格。
- `combo`:训练集未包含的龙 / 海盗狼同样继承目标游戏美术风格(高饱和、面板框架、UI 文字、金币氛围)。**风格 LoRA 可泛化至训练集外的新主体**。
...
- **对照展示**:mermaid 的 base→style→combo 三联 + dragon/pirate 的 base→combo 泛化对照,直观呈现"同一主体经 LoRA 后转为目标美术素材"。
